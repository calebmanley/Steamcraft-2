...
public class BlockNukeFurnace extends BlockContainer
{
	...
	protected BlockNukeFurnace(int id, boolean flag)
	{
		...
		/*
		if(this.blockID == ModBlocks.nukeOvenIdle.blockID)
		{
	...
	{
		this.blockIcon = icon.registerIcon("steamcraft:nukefurnaceside");
		this.furnaceIconFront = icon.registerIcon(this.isActive ? "steamcraft:nukefurnacefronton" : "steamcraft:nukefurnacefrontoff");
		this.furnaceIconTop = icon.registerIcon("steamcraft:nukefurnacetop");
	}
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer player, int l, float f1, float f2, float f3)
	{
		...
		if(furnace != null)
		{
			//TODO: ModLoader.OpenGUI(entityplayer, new GuiNukeFurnace(entityplayer.inventory, tileentitynukefurnace));
		}
